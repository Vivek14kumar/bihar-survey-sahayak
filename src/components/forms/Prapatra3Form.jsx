...
export default function Prapatra3Form({ isGenerating, onGenerate }) {
...
const updateApplicant = (index, field, value) => {
  ...
  const executePrint = useReactToPrint({ 
    contentRef: printRef, 
    documentTitle: "प्रपत्र_3_1_वंशावली",
  ...
  const selectSuggestion = (selectedWord, fieldName) => {
    setFormData((prev) => {
      ...
      words[words.length - 1] = selectedWord;
      // Added the missing '}' right before the semicolon
  ...
  return (
    <div className="p-2 md:p-6 max-w-[1400px] mx-auto bg-gray-100 flex flex-col gap-6 font-sans">
      ...
      {/* TOP: Input Form */}
      <div id="form-container" className="w-full bg-white p-4 md:p-6 shadow-xl rounded-xl border-t-[6px] border-blue-600 scroll-smooth print:hidden relative">
  ...
  <div className="bg-white p-3 rounded-xl border border-orange-100 shadow-sm mt-4">
    ...
    {renderInput({ label: "पुत्री(यों), पत्नी (का नाम)", name: "app3RelName" })}
        ...
        <div className="bg-yellow-50 p-4 md:p-5 rounded-3xl mb-5 border border-yellow-200 shadow-sm">
          ...
          {renderInput({ label: "जिसके नाम से ज़मीन है", name: "propertyOwnerName" })} {/**स्व......... (जिसके नाम से ज़मीन है) */}
          <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-green-50 p-4 md:p-5 rounded-3xl mb-6 border border-green-200 shadow-sm">
        ...
        {/* --- BOTTOM BUTTONS --- */}
        <div className="w-full z-20 bg-white rounded-3xl shadow-[0_0_20px_rgba(0,0,0,0.1)] px-4 py-5 border border-gray-200">
          <div className="grid grid-cols-2 gap-4">
            <button 
              onClick={() => processAction('print')} 
              disabled={isGeneratingPDF || isGenerating}
              className="relative flex items-center justify-center gap-1 bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-4 rounded-2xl font-bold text-[14px] hover:shadow-lg transition-all disabled:opacity-70"
            >
              <Printer size={20} /><span>{isGenerating ? 'loading...' : 'Print'}</span>
            </button>
            <button 
              onClick={() => processAction('download')} 
              disabled={isGeneratingPDF || isGenerating} 
              className="relative flex  items-center justify-center gap-1 bg-gradient-to-r from-yellow-400 to-amber-500 text-black py-4 rounded-2xl font-bold text-[14px] hover:shadow-lg transition-all disabled:opacity-70"
            >
              <Download size={20} /><span>{(isGeneratingPDF || isGenerating) ? 'Loading...' : 'PDF Download'}</span>
            </button>
          </div>
        </div>
      </div>
