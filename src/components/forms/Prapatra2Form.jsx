...
export default function Prapatra2Form({ isGenerating, onGenerate }) {
    ...
    setEntries(prev => prev.map(e => {
      if (e.id === entryId) {
        if (shareholderIndex !== null && shareholderIndex !== undefined) {
          ...
          newDetails[shareholderIndex][field] = words.join(" ") + " "; 
          return { ...e, ryotDetails: newDetails };
        }
  ...
  // 4. The Unified Gatekeeper for Print/Download utilizing Wallet Props
  const handleSecureAction = async (actionType) => {
    ...
    if (onGenerate) {
      ...
      // 1. Safely extract the primary client's name from your nested ryotDetails array
      // If they left it blank, it defaults to "Unknown Client" so the database doesn't crash
      const primaryClientName = entries[0].ryotDetails[0]?.name || "Unknown Client";
      
      // 2. Safely extract the mobile number
      const clientMobileNumber = entries[0].mobile || "";

      // 3. Pass them to the Dashboard's onGenerate function!
      // (Assuming you still pass actionType to differentiate buttons if needed, 
      // but you MUST pass the name and mobile for the ledger)
      await onGenerate(primaryClientName, clientMobileNumber);
    }

      // If the promise resolves, wallet deduction was successful, proceed to generate
      ...
      // Errors or Insufficient balance toasts should be handled by the parent component throwing the error.
    }
  };
  const firstKhata = entries[0].plots[0].khata || todayDate;
  const handlePrint = useReactToPrint({
    contentRef,
    documentTitle: `Prapatra-2_${'khata_',firstKhata || todayDate}`,
    onAfterPrint: async () => {
      setShowWatermark(true);
    },
  });
        ...
        <div ref={contentRef} className=" relative max-md:min-w-[1000px] mx-auto bg-white shadow-2xl print:shadow-none print:m-0 page-container p-4 md:p-6 lg:p-8 border border-gray-200 print:border-none w-full">
          ...
          <section className="grid grid-cols-3 gap-y-4 text-sm mb-4 text-black font-medium">
            {[
              ...
              { label: "थाना नं०", field: "thanaNo", width: "w-24", numeric: true }, 
              { label: "हल्का नं०", field: "halkaNo", width: "w-24", numeric: true }, 
                          ...
                          <>
                            <td className="border border-black p-1 align-top" rowSpan={entry.plots.length}>
                              ...
                              {entry.ryotDetails.length < 4 && (
                                ...
                                >
                                  + अन्य हिस्सेदार जोड़ें ({entry.ryotDetails.length}/4)
                                </button>
                              )}
                            </td>
                            <td className="border border-black p-1 align-top" rowSpan={entry.plots.length}>
            ...
            <div className="grid grid-cols-3 items-start px-4">
              <div className="space-y-0.5 text-sm font-medium">
                <p>
                  ...
                  <p>मोबाईल नं०: <input className="border-b border-dotted border-black outline-none w-32 font-bold bg-transparent placeholder:italic" value={entries[0].mobile}
                   onChange={(e) => {
                      // <-- FIX 2: Add onChange to update the state when they type
                      const newEntries = [...entries];
                      newEntries[0].mobile = e.target.value;
                      setEntries(newEntries);
                    }} maxLength={10} minLength={10} placeholder="लिखें"  />
                  </p>
              </div>
