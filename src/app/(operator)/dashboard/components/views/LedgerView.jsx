  ...
  const handleRequestRefund = async (docId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setGeneratedDocs(prevDocs => prevDocs.map(doc => (doc._id === docId || doc.id === docId) ? { ...doc, status: "REFUND_REQUESTED" } : doc));
      } else {
                  ...
                  {groupedDocs[date].map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-100 rounded-xl hover:border-blue-200 hover:shadow-sm transition-all gap-4">
                      ...
                      <div className="shrink-0 text-right flex flex-col items-end gap-1.5">
                        <span className={`text-xs font-black px-2.5 py-1.5 rounded-lg border ${doc.status === "REFUNDED" ? "text-slate-500 bg-slate-100 border-slate-200 line-through" : "text-emerald-600 bg-emerald-50 border-emerald-100"}`}>₹{doc.cost || "NA"}</span>
                        <div className="mt-2 text-right shrink-0">
                          {doc.status === "REFUND_REQUESTED" ? <span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-200 shadow-sm"><Clock size={10} className="animate-pulse" /> Review Pending</span>
                           : doc.status === "REFUNDED" ? <span className="inline-flex items-center gap-1 text-[9px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-200 shadow-sm"><CheckCircle2 size={10} /> Refunded</span>
                           : doc.status === "REFUND_REJECTED" ? <span className="inline-flex items-center gap-1 text-[9px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-200 shadow-sm"><XCircle size={10} /> Request Denied</span>
                           : <button onClick={() => handleRequestRefund(doc.id || doc._id)} className="inline-flex items-center gap-1 text-[10px] text-slate-400 hover:text-rose-500 font-medium underline underline-offset-2 transition-colors focus:outline-none"><AlertCircle size={10} /> Report Error</button>}
                        </div>
                      </div>
                    </div>
                  ))}
